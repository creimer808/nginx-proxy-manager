...
import { getSecurityEvent, getSecurityEvents, getSecurityFindings, getSecurityLogFiles, getSecurityLogLines, getSecurityOverview, getSecurityRules, getSecuritySettings, getSyslogExport } from "src/api/backend";
...
/** Polls while the panel is open so the delivery state stays current without a manual refresh. */
export const useSyslogExport = (enabled = true) => useQuery({ queryKey: ["security", "syslog"], queryFn: ({ signal }) => getSyslogExport(signal), enabled, refetchInterval: enabled ? 15_000 : false });
